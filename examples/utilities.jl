...
function myplot(sol, name="", analytic=false)
    N = length(sol.u[1])
    if analytic == true
        plot(sol,color=palette(:default)[1:2*N]',legend=:right,plot_analytic=true)
    else
        plot(sol,color=palette(:default)[1:N]',legend=:right,plot_analytic=false)
    end
    p = plot!(sol,color=palette(:default)[1:N]',denseplot=false,markershape=:circle,markerstrokecolor=palette(:default)[1:N]',linecolor = invisible(),label="")
    title!(name)
    return p
end