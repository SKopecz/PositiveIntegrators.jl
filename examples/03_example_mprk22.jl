...
# solutions with constant equispaced time steps
dt0 = 1.0
sol_nonlinmod_op_Tsit5 = solve(nonlinmod_op, Tsit5(), dt = dt0, abstol = 1e-2,
                               reltol = 1e-3);
sol_nonlinmod_op_MPRK22_1 = solve(nonlinmod_op, MPRK22(1.0), dt = dt0, abstol = 1e-2,
                                  reltol = 1e-3);
sol_nonlinmod_op_MPRK22_½ = solve(nonlinmod_op, MPRK22(0.5), dt = dt0, abstol = 1e-2,
                                  reltol = 1e-3);
...
# solutions with constant equispaced time steps
dt0 = 1.0
sol_nonlinmod_ip_Tsit5 = solve(nonlinmod_ip, Tsit5(), dt = dt0, abstol = 1e-2,
                               reltol = 1e-3);
sol_nonlinmod_ip_MPRK22_1 = solve(nonlinmod_ip, MPRK22(1.0), dt = dt0, abstol = 1e-2,
                                  reltol = 1e-3);
sol_nonlinmod_ip_MPRK22_½ = solve(nonlinmod_ip, MPRK22(0.5), dt = dt0, abstol = 1e-2,
                                  reltol = 1e-3);
...
# plot solution
p1 = plot(sol_robertson_op_RB23[2:end],
          idxs = [(0, 1), ((x, y) -> (x, 1e4 .* y), 0, 2), (0, 3)],
          color = palette(:default)[1:3]', legend = :right, xaxis = :log)
plot!(sol_robertson_op_RB23[2:end],
      idxs = [(0, 1), ((x, y) -> (x, 1e4 .* y), 0, 2), (0, 3)],
      color = palette(:default)[1:3]', denseplot = false, markershape = :circle,
      markerstrokecolor = palette(:default)[1:2]', linecolor = invisible(), label = "")
title!("Rosenbrock23")
p2 = plot(sol_robertson_op_MPRK22_1[2:end],
          idxs = [(0, 1), ((x, y) -> (x, 1e4 .* y), 0, 2), (0, 3)],
          color = palette(:default)[1:3]', legend = :right, xaxis = :log)
plot!(sol_robertson_op_MPRK22_1[2:end],
      idxs = [(0, 1), ((x, y) -> (x, 1e4 .* y), 0, 2), (0, 3)],
      color = palette(:default)[1:3]', denseplot = false, markershape = :circle,
      markerstrokecolor = palette(:default)[1:2]', linecolor = invisible(), label = "")
title!("MPRK22(1)")
...
# Test constant time step size
sol_robertson_ip_RB23 = solve(robertson_ip, Rosenbrock23(autodiff = false), abstol = 1e-3,
                              reltol = 1e-2);
